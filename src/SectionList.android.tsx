...
import { SectionList, SectionListProps, NativeModules, Platform } from 'react-native';
...
export default (React.forwardRef(
  <T extends any>(
    ...
    forwardedRef:
      | ((instance: SectionList<T> | null) => void)
      | MutableRefObject<SectionList<T> | null>
      | null
  ) => {
    const { maintainVisibleContentPosition: mvcp } = props;

    const flRef = useRef<SectionList<T> | null>(null);
    ...
    const refCallback: (instance: SectionList<T> | null) => void = (ref) => {
      flRef.current = ref;
    ...
    return <SectionList<T> {...props} ref={refCallback} />;
  }
